...
if ($db_conn) {
    ...
    if ($dist_id != '') {
        ...
        $p_data['sql'] = "select idx, u_business from member where id = ? ";
        $db_dataArr = $CASHAdminDAO->getQueryData_pre($p_data['sql'], [$dist_id]);
        $sel_dist_idx = $db_dataArr[0]['idx'];
        $where_new = " AND T1.recommend_member = ? ";
        array_push($param_where_new, $sel_dist_idx);
        // 하위 총판의 recommend_member 값을 얻어온다.
        $p_data['sql'] = "select idx from member where recommend_member = ? and u_business <> 1 ";
        $db_lowDist = $CASHAdminDAO->getQueryData_pre($p_data['sql'], [$sel_dist_idx]);
    ...
    $total_point = 0;
    $total_point_sub = 0;
    //GameCode::doSumChExCalc($shopConfig, $stats_day, $db_dataArr, $total_point, $total_point_sub, $tot_ch_val, $tot_ex_val);
    if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
        list($p_data['sql'], $param) = ComQuery::doComChExQuery($db_srch_s_date, $db_srch_e_date, $where_low, $param_where_low);
        $db_dataLowArr = $CASHAdminDAO->getQueryData_pre($p_data['sql'], $param);
        $db_dataLowArr = isset($db_dataLowArr) ? $db_dataLowArr : [];
    
        // 충전,환전 합산 구하기 
        GameCode::doSumLowChExCalc($shopConfig, $stats_day, $db_dataLowArr,$total_point_sub);
    }
    ...
    if ('' != $dist_id) {
        ...
        $param_dis[] = $sel_dist_idx; //$dist_id;
    }
    ...
    if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
        $where_low_dis = "AND parent.idx in(".$str_where_low_qm.")";
        $param = array_merge([$db_srch_s_date, $db_srch_e_date], $param_where_low);
        $p_data['sql'] = ComQuery::doSportsRealForderBetQuery($db_srch_s_date, $db_srch_e_date, $where_low_dis);
        $db_dataLowArr = $CASHAdminDAO->getQueryData_pre($p_data['sql'], $param);
        $db_dataLowArr = isset($db_dataLowArr) ? $db_dataLowArr : [];
        GameCode::doSumLowSportsRealBetCalc($db_dataLowArr, $stats_day, $shopConfig
                , $total_point_sub
        );
    }
    ...
    if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
        $param = array_merge([$db_srch_s_date, $db_srch_e_date], $param_where_low);
        array_push($param, $excluded_member_idx);
        $p_data['sql'] = ComQuery::doMiniBetQuery($db_srch_s_date, $db_srch_e_date, $excluded_member_idx, $where_low_dis);
        $db_dataArr = $CASHAdminDAO->getQueryData_pre($p_data['sql'], $param);
        $db_dataArr = isset($db_dataArr) ? $db_dataArr : [];
        GameCode::doSumLowMiniBetCalc($db_dataArr, $stats_day, $shopConfig,$total_point_sub);
    }
    ...
    $where_kplay = " AND PRT.idx = $sel_dist_idx ";
    ...
    if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
      $str_param_where_low = implode(',', $param_where_low);
      $where_low_dis = "AND PRT.idx in($str_param_where_low)";
      $p_data['sql'] = ComQuery::doCasinoByDistQuery($db_srch_s_date, $db_srch_e_date, $where_low_dis);
      $db_casinoDataArr = $CASHAdminDAO->getQueryData($p_data);
      $db_casinoDataArr = isset($db_casinoDataArr) ? $db_casinoDataArr : [];
      GameCode::doSumLowCasinoBetCalc($db_casinoDataArr, $stats_day, $shopConfig ,$total_point_sub);
    }
    ...
    if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
        $p_data['sql'] = ComQuery::doSlotByDistQuery($db_srch_s_date, $db_srch_e_date, $where_low_dis);
        $db_slotDataArr = $CASHAdminDAO->getQueryData($p_data);
        $db_slotDataArr = isset($db_slotDataArr) ? $db_slotDataArr : [];
        GameCode::doSumLowSlotBetCalc($db_slotDataArr, $stats_day, $shopConfig, $total_point_sub);
    }
    ...
    if ('ON' == IS_ESPORTS_KEYRON) {
        ...
        if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
            $p_data['sql'] = ComQuery::doEsptByDistQuery($db_srch_s_date, $db_srch_e_date, $where_low_dis);
            $db_esptDataArr = $CASHAdminDAO->getQueryData($p_data);
            $db_esptDataArr = isset($db_esptDataArr) ? $db_esptDataArr : [];
            GameCode::doSumLowEsptBetCalc($db_esptDataArr, $stats_day, $shopConfig, $total_point_sub);
        }
    }
    ...
    if ('ON' == IS_HASH) {
        ...
        if (0 < $sel_dist_idx && 0 < count($param_where_low)) {
            $p_data['sql'] = ComQuery::doHashByDistQuery($db_srch_s_date, $db_srch_e_date, $where_low_dis);
            $db_hashDataArr = $CASHAdminDAO->getQueryData($p_data);
            $db_hashDataArr = isset($db_hashDataArr) ? $db_hashDataArr : [];
            GameCode::doSumLowHashBetCalc($db_hashDataArr, $stats_day, $shopConfig,$total_point_sub);
        }
    }
