...
if($db_conn) {
    ...
    if($total_cnt > 0) {
        $p_data['sql'] = " SELECT idx, a_ip, a_memo, isuse, reg_time FROM t_adm_ip ";
...
<input type="hidden" name="monitor_charge" id="monitor_charge">
        ...
        <div class="panel search_box">
            <div style="color:#f89d1b!important">
            	※등록된 IP 에서만 관리자 페이지 접근이 가능하며, IP가 아무것도 등록되지 않는 경우 모든 IP 에서 접근 가능합니다.
            </div>
            ...
            <div class="panel_tit" style="margin-top: 5px;">
                <div class="" style="text-align: center">
                    <a href="javascript:;" onClick="setAdmIP('reg',0);" class="btn h30 btn_blu">등 록</a>
                </div>
            </div>
        </div>
        <!-- END detail search -->
...
?>                    
                    <tr onmouseover="this.style.backgroundColor='#FDF2E9';" onmouseout="this.style.backgroundColor='#ffffff';">
                        ...
                        <td width="200px"><?=$row['a_ip']?></td>
                        <td style="text-align: left"><?=$row['a_memo']?></td>
                        <td width="150px"><?=$row['reg_time']?></td>
                        <td width="80px" >
                        	<a href="javascript:;" onClick="setAdmIP('del','<?=$db_idx?>');" class="btn h30 btn_gray">삭제</a> 
                        </td>
                    </tr>
<?php        
            $i++;
...
function ValidateIPaddress(pVal) {
    let arr_ip = pVal.split('.');
    if(arr_ip[3] === '+'){
        pVal = arr_ip[0]+'.'+arr_ip[1]+'.'+arr_ip[2]+'.0';
    }
