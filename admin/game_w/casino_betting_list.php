...
//$p_data['game_type'] = trim(isset($_REQUEST['game_type']) ? $_REQUEST['game_type'] : '1');
    ...
    // 전체갯수
    $p_data['sql'] = "SELECT count(*) as CNT FROM KP_CSN_BET_HIST JOIN member ON KP_CSN_BET_HIST.MBR_IDX = member.idx";
    $p_data['sql'] .= " WHERE REG_DTM >= '" . $p_data['db_srch_s_date'] . "' AND REG_DTM <= '" . $p_data['db_srch_e_date'] . "'";
    ...
    // 소속 유저가 아니면 검색 안됨.
    if ($total_cnt > 0) {
        $p_data['sql'] = "SELECT KP_CSN_BET_HIST.*, id, nick_name FROM KP_CSN_BET_HIST JOIN member ON KP_CSN_BET_HIST.MBR_IDX = member.idx";
        $p_data['sql'] .= " WHERE REG_DTM >= '" . $p_data['db_srch_s_date'] . "' AND REG_DTM <= '" . $p_data['db_srch_e_date'] . "'";
        $p_data['sql'] .= $srch_basic;
        $p_data['sql'] .= " order by CSN_BET_IDX DESC";
    ...
    <div class="wrap">
        ...
        <div class="con_wrap">
                ...
                <div class="tline">
                    <table class="mlist">
                        ...
                        foreach ($db_dataArr as $key => $item) {
                            ...
                            //if($item['RSLT_MNY'] > 0)
                            //    $hld_money = $item['HLD_MNY'] + $item['RSLT_MNY'];
                            $gameName = true === isset($gameList[$item['GAME_ID']]) && false === empty($gameList[$item['GAME_ID']]) ? $gameList[$item['GAME_ID']] : '카지노게임';
                            ?>

                            <tr>
                                <td>
                                    <div class="checkbox checkbox-css checkbox-inverse" style="display:inline-block; text-align:center; width:20px; height:20px;" >
                                        <input type="checkbox" name="chk" id="<?= $chkbox_css[$i] ?>" value="<?= $item['CSN_BET_IDX'] ?>" data-member-idx="<?= $item['MBR_IDX'] ?>"/>
                                ...
                                <td width="5%"><?= $prdList[$item['PRD_ID']] ?></td>
                                <td width="5%"><?= $gameName ?></td>
                                <td width="5%" bgcolor="<?= $status_color ?>"><?= $status ?></td>
                                <td width="5%"><?= $item['TRX_ID'] ?></td>
                                <?php $i++;
                            }
                            ?>
                    </table>
            ...
            <div class="panel reserve hidden" data-main>
                <form id="search" name="search" data-search-form>
                    <div class="panel_tit">
                        <div class="search_form fl">
                            <?php
                            $format = 'Y-m-d';
                            $today = date($format);
                            $before_week = date($format, strtotime("-1 week", time()));
                            $before_month = date($format, strtotime("-1 month", time()));
                            $start_date = date($format, strtotime("-3 day", time()));
                            ?>
                            <div class="daterange">
                                <label for="datepicker-default"><i class="mte i_date_range mte-1x vat"></i></label>
                                <input type="text" class="" name="start_date" data-start-date-input id="datepicker-default" placeholder="날짜선택" value="<?= $_GET['start_date'] ?>" />
                            </div>
                            ~
                            <div class="daterange">
                                <label for="datepicker-autoClose"><i class="mte i_date_range mte-1x vat"></i></label>
                                <input type="text" class="" name="end_date" data-end-date-input id="datepicker-autoClose" placeholder="날짜선택" value="<?= $_GET['end_date'] ?>" />
                            </div>
                            <div><a href="javascript:BettingHistory.setDate('<?= $today ?>', '<?= $today ?>');" class="btn h30 btn_blu">오늘</a></div>
                            <div><a href="javascript:BettingHistory.setDate('<?= $before_week ?>', '<?= $today ?>');" class="btn h30 btn_orange">1주일</a></div>
                            <div><a href="javascript:BettingHistory.setDate('<?= $before_month ?>', '<?= $today ?>');" class="btn h30 btn_green">한달</a></div>
                            <div class="" style="padding-right: 10px;"></div>
                            <div>
                                <select name="game_id" style="width: 100%" data-games-select>
                                    <option value="0">전체</option>
                                </select>
                            </div>
                            <div class="" style="padding-right: 10px;">
                                <select name="search_type" id="srch_key">
                                    <option value="idnick">아이디 및 닉네임</option>
                                </select>
                            </div>
                            <div class="" style="padding-right: 10px;">
                                <input type="text" name="search_value" class="" placeholder="" value="" />
                            </div>
                            <div> <button type="submit" class="btn h30 btn_red">검색</button></div>
                            <?php if (0 == $u_business) { ?>
                                <div><a href="javascript:openNotePop();" style="margin-left: 10px" class="btn h30 btn_blu">쪽지</a></div>
                            <?php } ?>
                            <!--<div><a href="#" class="btn h30 btn_blu" onclick="onBtnClickTotalCalculate()">전체 정산</a></div>-->
                ...
                <div class="page" data-pagination></div>
            </div>
            <?php endif ?>
            <!-- END list -->
