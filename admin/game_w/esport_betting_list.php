...
// 관리자면 전체
   ...
   $srch_basic .= " AND member.recommend_member in ($str_param) ";
    ...
    // 전체갯수
    $p_data['sql'] = "SELECT count(*) as CNT FROM KP_ESPT_BET_HIST JOIN member ON KP_ESPT_BET_HIST.MBR_IDX = member.idx";
    $p_data['sql'] .= " WHERE PRD_ID = 101 AND REG_DTM >= '" . $p_data['db_srch_s_date'] . "' AND REG_DTM <= '" . $p_data['db_srch_e_date'] . "'";
    ...
    //if(!$find) $total_cnt = 0;
    if ($total_cnt > 0) {
        $p_data['sql'] = "SELECT KP_ESPT_BET_HIST.*, id, nick_name FROM KP_ESPT_BET_HIST JOIN member ON KP_ESPT_BET_HIST.MBR_IDX = member.idx";
        $p_data['sql'] .= " WHERE PRD_ID = 101 AND REG_DTM >= '" . $p_data['db_srch_s_date'] . "' AND REG_DTM <= '" . $p_data['db_srch_e_date'] . "'";
        $p_data['sql'] .= $srch_basic;
