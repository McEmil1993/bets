...
if ($db_conn) {
    ...
    // 전체갯수
    $p_data['sql'] = "SELECT count(*) as CNT FROM KP_SLOT_BET_HIST JOIN member ON KP_SLOT_BET_HIST.MBR_IDX = member.idx";
    ...
    //if(!$find) $total_cnt = 0;
    if ($total_cnt > 0) {
        $p_data['sql'] = "SELECT KP_SLOT_BET_HIST.*, id, nick_name FROM KP_SLOT_BET_HIST JOIN member ON KP_SLOT_BET_HIST.MBR_IDX = member.idx";
        $p_data['sql'] .= " WHERE REG_DTM >= '" . $p_data['db_srch_s_date'] . "' AND REG_DTM <= '" . $p_data['db_srch_e_date'] . "'";
        $p_data['sql'] .= $srch_basic;
        $p_data['sql'] .= " order by SLOT_BET_IDX DESC";
    ...
    // prd list
    $p_data['sql'] = "select PRD_ID, PRD_NM from KP_PRD_INF where TYPE='S'";
    $result = $LSportsAdminDAO->getQueryData($p_data);
                ...
                <div class="tline">
                    <table class="mlist">
                        ...
                        foreach ($db_dataArr as $key => $item) {
                            ...
                            //if($item['RSLT_MNY'] > 0)
                            //    $hld_money = $item['HLD_MNY'] + $item['RSLT_MNY'];
                            $gameName = true === isset($gameList[$item['GAME_ID']]) && false === empty($gameList[$item['GAME_ID']]) ? $gameList[$item['GAME_ID']] : '슬롯게임';
                            ?>
