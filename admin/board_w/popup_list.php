...
if ($db_conn) {
    ...
    $p_data['sql'] = " SELECT COUNT(*) AS CNT FROM popups a WHERE a.idx > 0 ";
    ...
    if ($total_cnt > 0) {
        $p_data['sql'] = " SELECT * ";
        $p_data['sql'] .= "     FROM popups a WHERE a.idx > 0 ";
...
<?php if ($p_data['vtype'] == 'node') {
    ...
    $node_btn = "btn_mdark";
                                ...
                                <div><a href="/board_w/popup_write.php" class="btn h30 <?= $all_btn ?>">팝업 등록</a></div>
                            </div>
                        </div>
                    </form>            
                    <div class="tline">
                                ...
                                if (!empty($db_dataArr)) {
                                    foreach ($db_dataArr as $row) {
                                        ...
                                        <tr onmouseover="this.style.backgroundColor = '#FDF2E9';" onmouseout="this.style.backgroundColor = '#ffffff';">
                                            ...
                                            <td>
                                                <a href="/board_w/popup_update.php?idx=<?= $db_m_idx ?>&status=<?= $row['status'] ?>&rank=<?= $row['rank'] ?>" class="btn h25 btn_blu">수정</a>
...
$default_link = "$reqFile?&status=" . $p_data['status'];
        ...
        function fn_del_event(idx) {
            ...
            if (result) {
                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: '/board_w/_popup_prc_del.php',
                    data: {'idx': idx},
