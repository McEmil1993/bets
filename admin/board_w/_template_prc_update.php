...
$idx = $_POST['idx'];
$type = trim(isset($_POST['type']) ? $BdsAdminDAO->real_escape_string($_POST['type']) : 0);
$division = trim(isset($_POST['division']) ? $BdsAdminDAO->real_escape_string($_POST['division']) : '');
...
if ($db_conn) {
    $p_data['sql'] = "UPDATE template SET `type` = $type, division = '$division', title = '$msg_title', content = '$msg_content', update_dt = NOW() WHERE idx = $idx";
