...
if($db_conn) {
	...
	$p_data['sql'] = "SELECT count(*) as cnt, thumbnail FROM popups WHERE thumbnail in (SELECT thumbnail FROM popups WHERE idx = $idx)";
	$db_dataArr = $BdsAdminDAO->getQueryData($p_data)[0];
	
	$p_data['sql'] = "delete from popups where idx = $idx";
