...
$thumbnail = 'notice/'.$p_data['filename'];

//$p_data['aid'] = 'asdf';

if($db_conn) {
    if (isset($p_data['filename']) && $p_data['filename'] != '') {
        $p_data['sql'] = "update notices set name = '$msg_title', thumbnail = '$thumbnail', detail = '$detail', status = $status where idx = $idx";
    }else{
        $p_data['sql'] = "update notices set name = '$msg_title', detail = '$detail', status = $status where idx = $idx";
    }
