...
if($db_conn) {
    ...
    $p_data['sql'] = " SELECT * FROM notices WHERE idx = ".$p_data['idx'];
...
<input type="hidden" id="autonum" name="autonum">
        ...
        <div class="panel reserve">
            ...
            <div class="tline">
                ...
                <table class="mlist">
                	<tr>
                    	<th style="width: 150px; text-align:left">공지명</th>
                        <td>
                        	<div class="confing_box">
                        		<input type="text" name="send_m_title" id="send_m_title" placeholder="공지명을 입력해 주세요." value="<?=$db_dataArr['name']?>"/>
                        	</div>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="file_thumb_section">
                        	...
                        	<div id="loading"></div>
                                ...
                                <input name="savePath" type=hidden value='/<?=IMAGE_PATH?>/notice'>
                    ...
                    <tr>
                        ...
                        <td>
                            <div id="loading"></div>
                            <textarea name="b_content" id="b_content" rows="5" cols="100" style="width:780px; height:350px; display:none;"><?=$db_dataArr['detail']?></textarea><br>
                        </td>
                    </tr>
                </table>                
            ...
            <div class="panel_tit">
            	<div>
                    ...
                    <a href="/board_w/notice_list.php" id="adm_btn_notice_cancel" class="btn h30 btn_green" style="color: white">목록</a>
                </div>
            </div>
        </div>
        <!-- END list -->
...
$(document).ready(function(){
	...
	$("#adm_btn_notice_send").click(function(){
            ...
            //msg_content = msg_content.replace(/&nbsp;/gi,"");//공백제거
            //msg_content = msg_content.replace("<br></p>","</p><br>");
            ...
            if (result){
                    ...
                    $.ajax({
                            type: 'post',
                            dataType: 'json',
                            url: '/board_w/_notice_prc_update.php',
                            data:{'idx':idx, 'msg_title':msg_title,'filename':filename,'detail':encodeURIComponent(msg_content), 'status':status},
                            success: function (result) {
                                console.log(result['retMsg']);
                                    if(result['retCode'] == "1000"){
                                            alert('등록하였습니다.');
                                            location.href="/board_w/notice_list.php";
                                            return;
                                    }else{
                                            alert(result['retMsg']);
                                            return;
