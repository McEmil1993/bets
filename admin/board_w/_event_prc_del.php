...
if($db_conn) {
    $p_data['sql'] = "SELECT count(*) as cnt, thumbnail FROM events WHERE thumbnail in (SELECT thumbnail FROM events WHERE idx = $idx)";
    $db_dataArr = $BdsAdminDAO->getQueryData($p_data)[0];
    
    $p_data['sql'] = "delete from events where idx = $idx";
