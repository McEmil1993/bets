...
$p_data['detail'] = trim(isset($_POST['detail']) ? $BbsAdminDAO->real_escape_string(urldecode($_POST['detail'])) : '');
$p_data['status'] = trim(isset($_POST['status']) ? $BbsAdminDAO->real_escape_string($_POST['status']) : '');
$p_data['thumbnail'] = 'event/'.$p_data['filename'];

    
    $in_data = "values ('".$p_data['msg_title']."', '".$p_data['thumbnail']."', '".$p_data['detail']."', ".$p_data['status'].", 0, now())";

    $p_data['sql'] = "INSERT INTO events (name, thumbnail, detail, status, del_flag, create_dt) $in_data ";
