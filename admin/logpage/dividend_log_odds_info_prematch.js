class DividendLogCheck
{
    ...
    {
        this.table_data         = [];
        this.column_data        = ['league_id', 'kstStartDate', 'display_status','fixture_participants_1_id', 'fixture_participants_2_id', 'display_status', 'last_update', 'msg_guid', 'received','created_date_formatted'];   
    ...
    async loadTableData(append_mode = false)
    {
        ...
        $.ajax(
        {
            ...
            data:
            {
                action              : 'load_odds_info_prematch',
    ...
    async __loadTableData(append_mode = false)
    {
                ...
                for(let column of this.column_data)
                {
                    if(column == "received")
                    {
                        if(data[column])
                        {
                            ...
                            table_data_html += this.__tableTD("received");
                        }   
                        else
                        {
        ...
        {
            table_data_html += "<tr><td colspan='9'>No Result</td></tr>";
            this.last_page = true;
        }
