class DividendLogCheck
{
    ...
    {
        this.table_data         = [];
        this.column_data        = ['bet_name', 'bet_line', 'bet_price', 'tempPrice','bet_status', 'msg_guid', 'deduction_rate', 'providers', 'created_date_formatted'];   
        ...
        if(this.table_data)
        {
            for(let data of this.table_data)
            {
                ...
                for(let column of this.column_data)
                {
                    if(column == "providers")
                    {
                        console.log(JSON.parse(data[column]))
                        let provider_info = JSON.parse(data[column]);
    
                        if(provider_info.hasOwnProperty('providerId'))
                        {
                            table_data_html += this.__tableTD(provider_info.providerId);
                        }
                        else
                        {
