...
if($db_list > 0) {
    ...
    {
        $gameName = true === isset($gameList[$db_list[$i]['GAME_ID']]) && false === empty($gameList[$db_list[$i]['GAME_ID']]) ? $gameList[$db_list[$i]['GAME_ID']] : '잘못된데이터';
        $status = '배팅';
        $status_color = '';
        if('W' == $db_list[$i]['TYPE']){
            $status = '적중';
        ...
        //if($db_list[$i]['RSLT_MNY'] > 0)
        //    $hld_money = $db_list[$i]['HLD_MNY'] + $db_list[$i]['RSLT_MNY'];
        $gameName = true === isset($gameList[$db_list[$i]['GAME_ID']]) && false === empty($gameList[$db_list[$i]['GAME_ID']]) ? $gameList[$db_list[$i]['GAME_ID']] : '슬롯게임';
