...
if($db_conn) {
    ...
    $p_data['sql'] = "SELECT member_bet.*, member.idx as member_idx, member.id, member.nick_name FROM member_bet";
    ...
    $p_data['sql'] .= " WHERE member_bet.bet_type = 2 ";
    $p_data['sql'] .= "AND member.idx = ".$p_data['m_idx'];
...
<td width='7%' style='background-color:#6F6F6F;color:#fff'>게임수</td>
<td width='16%' style='background-color:#6F6F6F;color:#fff'>배팅진행내역</td>
<td width='12%' style='background-color:#6F6F6F;color:#fff'>배당율(보너스)</td>
<td width='8%' style='background-color:#6F6F6F;color:#fff'>배팅액</td>
<td width='8%' style='background-color:#6F6F6F;color:#fff'>예상당첨액</td>
<td width='8%' style='background-color:#6F6F6F;color:#fff'>적중금</td>
...
if($db_dataArr > 0) {
    foreach ($db_dataArr as $key => $item) {
        ...
        $data_str_2 .="<td width='7%'>".$gameCount."</td>";
        $data_str_2 .="<td width='16%' id='open_betting_detail_".$item['idx']."' onClick='open_betting_detail(".$item['idx'].",".$item['is_open'].")'>";
        ...
        $totalBetPrice = !empty($totalBetPrice) ? number_format($totalBetPrice,2) : 0;
        $bonusPrice = '';
        if($item['bonus_price'] > 0) $bonusPrice = '('.$item['bonus_price'].')';
        $data_str_2 .="<td width='12%' style='text-align:center'>".$totalBetPrice.$bonusPrice."</td>";
        $data_str_2 .="<td width='8%' style='text-align:right'>".$total_bet_money."</td>";
        ...
        $data_str_2 .="<td width='8%' style='text-align:right'>".$rating."</td>";
        $data_str_2 .="<td width='8%' style='text-align:right; ".$take_money_color."'>".number_format($item['take_money'])."</td>";
        ...
        foreach ($result_row as $key => $item2) {
            ...
            $data_str_2 .="<td><a target='_blank' href='".$protocol.$_SERVER['HTTP_HOST']."/sports_w/realtime_manager_detail.php?fixture_start_date=".urlencode($item2['fixture_start_date'])."&fixture_id=".$item2['ls_fixture_id']."'>".$team1."</a></td>";
            $data_str_2 .="<td>".$item2['bet_price']."</td>";
            $data_str_2 .="<td><a target='_blank' href='".$protocol.$_SERVER['HTTP_HOST']."/sports_w/realtime_manager_detail.php?fixture_start_date=".urlencode($item2['fixture_start_date'])."&fixture_id=".$item2['ls_fixture_id']."'>".$team2."</a></td>";
