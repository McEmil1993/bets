...
if($db_list > 0) {
    for ($i=0; $i<count($db_list); $i++)
    {
        $status = '게임중';
        $status_color = '';
        if('ROUND_END' == $db_list[$i]['EVENT'] && 0 < $db_list[$i]['WIN_MONEY']){
            $status = '적중';
            $status_color = '#9FD5E9';
        }else if('ROUND_END' == $db_list[$i]['EVENT']){
            $status = '낙첨';
            $status_color = '#F9BFBF';
        }
        
        // 당첨금
        $winMoney = $db_list[$i]['WIN_MONEY'];

        // 보유금액
        $final_money = $db_list[$i]['FINAL_MONEY'];
        
        $fee = $db_list[$i]['FEE'];
        
        $data_str_2 .="<tr><td style='width: 10%;'>".$db_list[$i]['REG_DTM']."</td>";
        $data_str_2 .="<td style='width: 5%; text-align:right;'>".$db_list[$i]['GAME_NUM']."</td>";
        $data_str_2 .="<td style='width: 5%; text-align:right;'>".number_format($db_list[$i]['BET_MONEY'])."</td>";
        $data_str_2 .="<td style='width: 5%; text-align:right;'>".number_format($winMoney)."</td>";
        $data_str_2 .="<td style='width: 5%; text-align:right;'>".number_format($fee)."</td>";
        $data_str_2 .="<td style='width: 5%; text-align:right;'>".number_format($final_money)."</td>";
