...
$p_member_data['memberIdxList'] = trim(isset($_POST['memberIdxList']) ? $_POST['memberIdxList'] : 0);
...
if ($db_conn) {
    ...
    $p_member_data['memberIdxList'] = $MEMAdminDAO->real_escape_string($p_member_data['memberIdxList']);
    
    $db_dataArr_msg_set = $MEMAdminDAO->getTemplateList(0); // 쪽지
    
    $p_data['sql'] = "select idx, id, nick_name from member where idx in (" . $p_member_data['memberIdxList'] . ")";
    ...
    $last_key = key($db_data_mem);
    foreach ($db_data_mem as $key => $value) {

        if ($key == $last_key) {
            $memberIdxList .= $value['idx'];
        } else {
            $memberIdxList .= $value['idx'] . ",";
        }
    }
    ...
    <body>
        <div class="wrap">
            <form id="regform" name="regform" method="post">
                ...
                <input type="hidden" id="memberIdxList" name="memberIdxList" value="<?= $memberIdxList ?>">
                <!-- Contents -->
                <div class="">
                    <!-- list -->
                    <div class="panel reserve" style="min-width: 960px; padding: 10px;">
                        ...
                        <div class="tline">
                            <table class="mlist">
                                <tr>
                                    <th style="width: 150px; text-align:left">받는 사람 (<?= count($db_data_mem) ?>명)</th>
        ...
        ?>
                        ...
                        <div class="panel_tit" style="text-align: center;">
                            <div class="clx modal_foot">
                                <a href="javascript:;" id="adm_btn_msg_send_list_pop" class="btn h30 btn_blu" data-dismiss="modal">쪽지 발송</a>
