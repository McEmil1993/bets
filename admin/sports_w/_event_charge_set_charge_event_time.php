...
try {
    $start_dt = $ALBetDAO->real_escape_string($start_dt);
    $end_dt = $ALBetDAO->real_escape_string($end_dt);
    
    $result['retCode'] = '1000';
    
    $sql = "UPDATE t_game_config SET set_type_val = '$start_dt' WHERE set_type = 'event_charge_start'";
    if (FAIL_DB_SQL_EXCEPTION ===  $ALBetDAO->executeQuery($sql)) {
        throw new mysqli_sql_exception('mysqli_sql_exception!!!');
    }
    
    $sql = "UPDATE t_game_config SET set_type_val = '$end_dt' WHERE set_type = 'event_charge_end'";
    if (FAIL_DB_SQL_EXCEPTION ===  $ALBetDAO->executeQuery($sql)) {
        throw new mysqli_sql_exception('mysqli_sql_exception!!!');
    }
    ...
    $log_data = '이벤트 충전 설정 : start_dt=>' . $start_dt . ' end_dt=>' . $end_dt;
    ...
    $p_data['sql'] .= " values('$admin_id','$now_ip','$st_country','$log_data',36) ;";
    ...
    CommonUtil::logWrite('_event_charge_set_charge_event_time to 2' . $e->getMessage(), "db_error");
    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
    
}catch (\Exception $e) {
    $UTIL->logWrite("[_event_charge_set_charge_event_time] [error -2]", "error");
    $result['retCode'] = -3;
    $result['retMsg'] = 'Exception 예외발생';
    
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('::::::::::::::: _event_charge_set_charge_event_time to 2 ReflectionException : ' . $e, "error");
    $result['retCode'] = -4;
    $result['retMsg'] = 'Exception 예외발생';
} finally {
    $ALBetDAO->dbclose();
