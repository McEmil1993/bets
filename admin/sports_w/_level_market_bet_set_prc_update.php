...
if ($db_conn) {
    ...
    $sql = "UPDATE level_bet_policy SET amount = $pre_amount, update_dt = NOW() WHERE level = $rank AND `bet_type` = 1";
    $LSportsAdminDAO->executeQuery($sql);

    $sql = "UPDATE level_bet_policy SET amount = $real_amount, update_dt = NOW() WHERE level = $rank AND `bet_type` = 2";
    $LSportsAdminDAO->executeQuery($sql);
