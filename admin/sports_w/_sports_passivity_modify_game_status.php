...
try {
    ...
    if (!$db_conn) {
        CommonUtil::logWrite("_sports_passivity_modify_game_status db_conn ", "error");
    ...
    if (false == $is_trans_start) {
        CommonUtil::logWrite("_sports_passivity_modify_game_status", "error");
        $result['retCode'] = FAIL_TRANS_START;
        $result['retMsg'] = FAIL_TRANS_START_MSG;
        return;
    }

    if (false === isset($_POST['fixture_id']) || false === isset($_POST['bet_type']) || false === isset($_POST['status'])) {
        $UTIL->logWrite("fail _sports_passivity_modify_game_status param ", "error");
    ...
    $p_data['sql'] = " SELECT IF('ON' = lsports_fixtures.passivity_flag AND lsports_fixtures.display_status_passivity is NOT NULL ,lsports_fixtures.display_status_passivity,lsports_fixtures.display_status) as display_status FROM lsports_fixtures WHERE fixture_id = $fixture_id AND bet_type = $bet_type";
    ...
    $p_data['sql'] = " UPDATE lsports_fixtures SET display_status_passivity = $status WHERE fixture_id = $fixture_id AND bet_type = $bet_type";
    ...
    $log_data = '경기 상태값 수동 변경 :  fixture_id =>' . $fixture_id . ' bet_type=>' . $bet_type . ' bf display_status=>' . $result_data[0]['display_status']
            .' af display_status=>' . $status;
    ...
    $log_type = PASSIVITY_MODIFY_STATUS;
    ...
    CommonUtil::logWrite('fail _sports_passivity_modify_game_status mysqli_sql_exception ' . $e->getMessage(), "db_error");

    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
} catch (\Exception $e) {
    $UTIL->logWrite("fail _sports_passivity_modify_game_status Exception ", "error");

    $result['retCode'] = FAIL_EXCEPTION;
    $result['retMsg'] = FAIL_EXCEPTION_MSG;
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('fail _sports_passivity_modify_game_status ReflectionException : ' . $e, "error");
