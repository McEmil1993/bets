...
if ($db_conn) {
    ...
    $p_data["sql_where"] = "";
    $default_link = 'prematch_manager.php?srch_s_date=' . $p_data['db_srch_s_date'];
    $s_id = "";
    $l_id = "";
    $fix_id = "";
    $bs_id = 1;
    ...
    if (isset($_GET['s_id'])) {
        $_GET['s_id'] = $MEMAdminDAO->real_escape_string($_GET['s_id']);
        $s_id = $_GET['s_id'];
        if (strlen($s_id) > 0) {
    ...
    if (isset($_GET['fix_id'])) {
        $_GET['fix_id'] = $MEMAdminDAO->real_escape_string($_GET['fix_id']);
        $fix_id = $_GET['fix_id'];
        if (strlen($fix_id) > 0) {
    ...
    if (isset($_GET['location_id'])) {
        $_GET['location_id'] = $MEMAdminDAO->real_escape_string($_GET['location_id']);
    ...
    if (isset($_REQUEST['l_id'])) {
        $_REQUEST['l_id'] = $MEMAdminDAO->real_escape_string($_REQUEST['l_id']);
        $l_id = $_REQUEST['l_id'];
    ...
    if (isset($_REQUEST['bs_id'])) {
        $_REQUEST['bs_id'] = $MEMAdminDAO->real_escape_string($_REQUEST['bs_id']);
        ...
        $default_link .= '&bs_id=' . $bs_id;
        $p_data["sql_where"] .= " AND IF('ON' = fix.passivity_flag AND fix.display_status_passivity is NOT NULL ,fix.display_status_passivity,fix.display_status) = 1";
    }
    if (isset($_REQUEST['tn'])) {
        $_REQUEST['tn'] = $MEMAdminDAO->real_escape_string($_REQUEST['tn']);
        $tn = $_REQUEST['tn'];
        if (strlen($tn) > 0) {
            $default_link .= '&tn=' . $tn;
    ...
    $total_cnt = $MEMAdminDAO->getSportsFixturesCount(1, $p_data, $startTime, $endTime);
    ...
    if ($total_cnt > 0) {
        $db_dataArr = $MEMAdminDAO->getSportsFixturesList(1, $p_data, $startTime, $endTime, $bs_id);
    }

    $sp = $MEMAdminDAO->getSportsList();

    $s = $MEMAdminDAO->getLeaguesList($s_id, 1);

    //CommonUtil::logWrite("srch_s_date REQUEST: " .json_encode($s), "error");
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.search_btn').on('click', function () {
                ...
                location.href = '/sports_w/prematch_manager.php?s_id=' + sport_id + '&l_id=' + league_id + '&bs_id=' + bet_status_id + '&tn=' + team_name + '&fix_id=' + fix_id + '&srch_s_date=' + srch_s_date;
            });
        ...
        <div class="wrap">
            ...
            <div class="con_wrap">
                ...
                <div class="panel search_box">
                    <h5><a href="/sports_w/prematch_manager.php"><b>프리매치(자동)</b></a></h5>
                    <h5><a href="/sports_w/prematch_manager_passivity.php">프리매치(수동)</a></h5>
                </div>
                <!-- END detail search -->
                    ...
                    <div class="panel_tit">
                        <div class="search_form fl">
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="sport_list" id="sport_list" onchange="fn_On_chnage('prematch_manager.php');">
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="location_list" id="location_list" onchange="fn_On_chnage('prematch_manager.php');">
                                    <option value="">전체</option>
                                    ...
                                    foreach ($db_dataArr as $key => $item) {
                                        if (!in_array($item['fixture_location_id'], $checkLocation)) {
                                            ?>
                                            <option 
                                                value="<?= $item['fixture_location_id'] ?>"
                                                <?php if ($location_id == $item['fixture_location_id']): ?> selected<?php endif; ?>
                                                data-image="../assets_admin/images/flag/<?= $item['fixture_location_id'] ?>.png">
                                                    <?= $item['location_name'] ?>
                                            </option>
                                            <?php
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="league_list" id="league_list" class="league_list_select" onchange="fn_On_chnage('prematch_manager.php');">
                                    <option value="">전체</option>
                                    <?php $checkLeague = array(); // 중복출력 체크용도 ?>
                                    <?php
                                    foreach ($db_dataArr as $key => $item) {
                                        if (!in_array($item['fixture_league_id'], $checkLeague)) {
                                            ?>
                                            ...
                                            <?php
                                            array_push($checkLeague, $item['fixture_league_id']);
                                        }
                                        ?>
                                    <?php } ?>
                                </select>
                            ...
                            <div class="" style="padding-right: 10px;">
                                <select name="bet_status_list" id="bet_status_list" onchange="fn_On_chnage('prematch_manager.php');">
                                    <option value="1" <?php if ($bs_id == 1): ?> selected<?php endif; ?>>배팅가능</option>
                                    <option value="2" <?php if ($bs_id == 2): ?> selected<?php endif; ?>>배팅마감</option>
                        ...
                        <div class="search_form fr">
                            <a href="#" class="btn h30 btn_dblu mg_l-10" onClick="fn_all_update_passivity_manager(1, 'ON', 'prematch_manager.php')">수동 관리</a>
                        </div>
                    </div>
                                    ...
                                    if (!empty($db_dataArr)) {
                                        foreach ($db_dataArr as $row) {
                                            ...
                                            <tr onmouseover="this.style.backgroundColor = '#FDF2E9';" onmouseout="this.style.backgroundColor = '#ffffff';">
                                                ...
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['fixture_id'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['fixture_start_date'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['sports_display_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['league_display_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= !empty($row['p1_display_name']) ? $row['p1_display_name'] : $row['p1_team_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= !empty($row['p2_display_name']) ? $row['p2_display_name'] : $row['p2_team_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')"><?= $row['live_results_p1'] ?> : <?= $row['live_results_p2'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" style='text-align:right'><?= ($row['total_bet_money'] == null) ? 0 : number_format($row['total_bet_money']) ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" style='text-align:right'><?= ($row['bet_id_total_bet_money'] == null) ? 0 : number_format($row['bet_id_total_bet_money']) ?></td>
                                                ...
                                                <td>
                                                    ...
                                                    <?php
                                                    $status_name = '';
                                                    $status_name = GameStatusUtil::get_display_stauts_name($row['display_status'], 1);
                                                    ?>
                                                    <?= $status_name ?>
                                                </td>
                                                <td>
                                                    <?php if ($row['admin_bet_status'] == 'ON') { ?>
                                                        <a href="#" onclick="betOnOffBtnClick(this,<?= $row['fixture_id'] ?>, 'off', '<?= $row['fixture_start_date'] ?>', 1)" class="btn h30 btn_green"> <?= $row['admin_bet_status'] ?></a>
                                                    <?php } else if ($row['admin_bet_status'] == 'OFF') { ?>
                                                        <a href="#" onclick="betOnOffBtnClick(this,<?= $row['fixture_id'] ?>, 'on', '<?= $row['fixture_start_date'] ?>', 1)" class="btn h30 btn_gray"> <?= $row['admin_bet_status'] ?></a>
                                                    <?php } ?>
