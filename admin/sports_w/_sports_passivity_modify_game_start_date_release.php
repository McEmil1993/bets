...
try {
    ...
    if (!$db_conn) {
        CommonUtil::logWrite("_sports_passivity_modify_game_start_date_release db_conn ", "error");
    ...
    if (false == $is_trans_start) {
        CommonUtil::logWrite("_sports_passivity_modify_game_start_date_release", "error");
    ...
    if (false === isset($_POST['fixture_id']) || false === isset($_POST['bet_type'])) {
        $UTIL->logWrite("fail _sports_passivity_modify_game_start_date_release param ", "error");
    ...
    $p_data['sql'] = " UPDATE lsports_fixtures SET fixture_start_date_passivity = default WHERE fixture_id = $fixture_id AND bet_type = $bet_type";
    ...
    $log_data = '경기 시작 시간 수동 적용 해제 :  fixture_id =>' . $fixture_id . ' bet_type=>' . $bet_type;
    ...
    $log_type = PASSIVITY_MODIFY_START_DATE_RELEASE;
    ...
    CommonUtil::logWrite('fail _sports_passivity_modify_game_start_date_release mysqli_sql_exception ' . $e->getMessage(), "db_error");

    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
} catch (\Exception $e) {
    $UTIL->logWrite("fail _sports_passivity_modify_game_start_date_release Exception ", "error");

    $result['retCode'] = FAIL_EXCEPTION;
    $result['retMsg'] = FAIL_EXCEPTION_MSG;
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('fail _sports_passivity_modify_game_start_date_release ReflectionException : ' . $e, "error");
