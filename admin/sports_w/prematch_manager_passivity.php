...
if ($db_conn) {
    ...
    $p_data["sql_where"] = "";
    $default_link = 'prematch_manager_passivity.php?srch_s_date=' . $p_data['db_srch_s_date'];
    $s_id = "";
    $l_id = "";
    $fix_id = "";
    $bs_id = 1;
    ...
    $total_cnt = $MEMAdminDAO->getSportsFixturesCount(1, $p_data, $startTime, $endTime, 'ON');
    ...
    if ($total_cnt > 0) {
        $db_dataArr = $MEMAdminDAO->getSportsFixturesList(1, $p_data, $startTime, $endTime, $bs_id, 'ON');
    }
...
<html lang="ko">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.search_btn').on('click', function () {
                ...
                location.href = '/sports_w/prematch_manager_passivity.php?s_id=' + sport_id + '&l_id=' + league_id + '&bs_id=' + bet_status_id + '&tn=' + team_name + '&fix_id=' + fix_id + '&srch_s_date=' + srch_s_date;
            });
    ...
    <body>
        ...
        <div class="wrap">
            <?php
            $menu_name = "prematch_manager_passivity";
            ...
            <div class="con_wrap">
                ...
                <div class="panel search_box">
                    <h5><a href="/sports_w/prematch_manager.php">프리매치(자동)</a></h5>
                    <h5><a href="/sports_w/prematch_manager_passivity.php"><b>프리매치(수동</b>)</a></h5>
                </div>
                <!-- END detail search -->
                    ...
                    <div class="panel_tit">
                        <div class="search_form fl">
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="sport_list" id="sport_list" onchange="fn_On_chnage('prematch_manager_passivity.php');">
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="location_list" id="location_list" onchange="fn_On_chnage('prematch_manager_passivity.php');">
                                    <option value="">전체</option>
                            ...
                            <div class="" style="padding-right: 5px;">
                                <select name="league_list" id="league_list" class="league_list_select" onchange="fn_On_chnage('prematch_manager_passivity.php');">
                                    <option value="">전체</option>
                            ...
                            <div class="" style="padding-right: 10px;">
                                <select name="bet_status_list" id="bet_status_list" onchange="fn_On_chnage('prematch_manager_passivity.php');">
                        ...
                        <div class="search_form fr">
                            <a href="#" class="btn h30 btn_green mg_l-10" onClick="fn_all_update_passivity_manager(1, 'OFF', 'prematch_manager_passivity.php')">자동 관리</a>
                        </div>
                    </div>
                                    ...
                                    if (!empty($db_dataArr)) {
                                        foreach ($db_dataArr as $row) {
                                            ...
                                            <tr onmouseover="this.style.backgroundColor = '#FDF2E9';" onmouseout="this.style.backgroundColor = '#ffffff';">
                                                ...
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['fixture_id'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['fixture_start_date'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['sports_display_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= $row['league_display_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= !empty($row['p1_display_name']) ? $row['p1_display_name'] : $row['p1_team_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" ><?= !empty($row['p2_display_name']) ? $row['p2_display_name'] : $row['p2_team_name'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')"><?= $row['live_results_p1'] ?> : <?= $row['live_results_p2'] ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" style='text-align:right'><?= ($row['total_bet_money'] == null) ? 0 : number_format($row['total_bet_money']) ?></td>
                                                <td onclick="fn_On_detail('/sports_w/prematch_manager_passivity_detail.php?fixture_start_date=<?= urlencode($row['fixture_start_date']) ?>&fixture_id=<?= $row['fixture_id'] ?>')" style='text-align:right'><?= ($row['bet_id_total_bet_money'] == null) ? 0 : number_format($row['bet_id_total_bet_money']) ?></td>
