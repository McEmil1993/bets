...
try {
    ...
    if (!$db_conn) {
        CommonUtil::logWrite("sports_menu_passivity_all_release_ajax db_conn ", "error");
    ...
    if (false == $is_trans_start) {
        CommonUtil::logWrite("sports_menu_passivity_all_release_ajax", "error");
        $result['retCode'] = FAIL_TRANS_START;
        $result['retMsg']  = FAIL_TRANS_START_MSG;
        return;
    }

    if (false == isset($_POST['fixture_id']) || false == isset($_POST['bet_type']) || false == isset($_POST['bet_data'])) {

        $UTIL->logWrite("[sports_menu_passivity_all_release_ajax] [error 2201]", "error");
    ...
    foreach ($arr_bet_data as $bet) {
        ...
        // 해당 정보를 defalut 값으로 업데이트 한다.
        $p_data['sql'] = "update lsports_bet set bet_status_passivity = default, bet_price_passivity = default where fixture_id = $fixture_id and bet_type = $bet_type and markets_id = $markets_id";
    ...
    $log_data = '마켓 상태 전체 수동 해제 :  fixture_id =>' . $fixture_id . ' bet_type=>' . $bet_type.' bet_data => '.$bet_data ;;
    ...
    $log_type = PASSIVITY_ALL_RELEASE;
    ...
    CommonUtil::logWrite('fail sports_menu_passivity_all_release_ajax mysqli_sql_exception ' . $e->getMessage(), "db_error");

    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
} catch (\Exception $e) {
    $UTIL->logWrite("fail sports_menu_passivity_all_release_ajax Exception ", "error");

    $result['retCode'] = FAIL_EXCEPTION;
    $result['retMsg'] = FAIL_EXCEPTION_MSG;
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('fail sports_menu_passivity_all_release_ajax ReflectionException : ' . $e, "error");
