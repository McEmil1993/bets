...
try {
    ...
    if (!$db_conn) {
        CommonUtil::logWrite("sports_menu_passivity_release_ajax db_conn ", "error");
    ...
    if (false == $is_trans_start) {
        CommonUtil::logWrite("sports_menu_passivity_release_ajax", "error");
        $result['retCode'] = FAIL_TRANS_START;
        $result['retMsg'] = FAIL_TRANS_START_MSG;
        return;
    }

    if (false == isset($_POST['fixture_id']) || false == isset($_POST['markets_id']) || false == isset($_POST['bet_type']) ) {

        $UTIL->logWrite("[sports_menu_passivity_release_ajax] [error 2201]", "error");
    ...
    $log_data = '마켓 상태 배당 수동 해제 :  fixture_id =>' . $fixture_id . ' bet_type=>' . $bet_type . ' markets_id=>' . $markets_id
    ...
    CommonUtil::logWrite('fail sports_menu_passivity_release_ajax mysqli_sql_exception ' . $e->getMessage(), "db_error");

    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
} catch (\Exception $e) {
    $UTIL->logWrite("fail sports_menu_passivity_release_ajax Exception ", "error");

    $result['retCode'] = FAIL_EXCEPTION;
    $result['retMsg'] = FAIL_EXCEPTION_MSG;
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('fail sports_menu_passivity_release_ajax ReflectionException : ' . $e, "error");
