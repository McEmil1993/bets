...
try {
    ...
    if (!$db_conn) {
        CommonUtil::logWrite("sports_menu_passivity_all_apply_ajax db_conn ", "error");
    ...
    if (false == $is_trans_start) {
        CommonUtil::logWrite("sports_menu_passivity_all_apply_ajax", "error");
    ...
    if (false == isset($_POST['fixture_id']) || false == isset($_POST['bet_type']) || false == isset($_POST['bet_data'])) {
        ...
        $UTIL->logWrite("[sports_menu_passivity_all_apply_ajax] [error 2201]", "error");
    ...
    foreach ($arr_bet_data as $bet) {
        ...
        $status = $bet['status'];
        $name = $bet['name'];
        $markets_id = $bet['market_id'];
        $bet_base_line = $bet['bet_base_line'];
        
        CommonUtil::logWrite('sports_menu_passivity_all_apply_ajax bet_price ' . $bet_price, "error");

         
        if(true === isset($bet_price) && false === empty($bet_price) && null != $bet_price){
    ...
    $log_data = '마켓 전체 수동 변경 :  fixture_id =>' . $fixture_id . ' bet_type=>' . $bet_type .' bet_data => '.$bet_data ;
    ...
    CommonUtil::logWrite('fail sports_menu_passivity_all_apply_ajax mysqli_sql_exception ' . $e->getMessage(), "db_error");

    $result['retCode'] = FAIL_DB_SQL_EXCEPTION;
    $result['retMsg'] = FAIL_DB_SQL_EXCEPTION_MSG;
} catch (\Exception $e) {
    $UTIL->logWrite("fail sports_menu_passivity_all_apply_ajax Exception ", "error");

    $result['retCode'] = FAIL_EXCEPTION;
    $result['retMsg'] = FAIL_EXCEPTION_MSG;
} catch (\ReflectionException $e) {
    CommonUtil::logWrite('fail sports_menu_passivity_all_apply_ajax ReflectionException : ' . $e, "error");
