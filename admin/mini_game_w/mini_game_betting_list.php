...
if('ON' == IS_EOS_POWERBALL)
    $default_game_type = 'eospb5';
else if('ON' == IS_POWERBALL)
    $default_game_type = 'powerball';
else
    $default_game_type = 'pladder';
...
$p_data['game_type'] = trim(isset($_REQUEST['game_type']) ? $_REQUEST['game_type'] : $default_game_type);
$p_data['betting_key'] = trim(isset($_REQUEST['betting_key']) ? $_REQUEST['betting_key'] : 0);

$srch_basic = "";
//switch($p_data["srch_key"]) {
switch($p_data["game_type"]) {
    case "kladder":
        $srch_basic = " b.bet_type = 5 ";
        break;
    case "pladder":
        $srch_basic = " b.bet_type = 4 ";
        break;
    case "powerball":
        $srch_basic = " b.bet_type = 15 ";
        break;
    case "eospb5":
        $srch_basic = " b.bet_type = 3 ";
        break;
    default:
        $srch_basic = " b.bet_type in(3,4,5,15) ";
        break;
}
            ...
            <div class="panel_tit">
            	<div class="search_form fl">
                    ...
                    <div class="" style="padding-right: 10px;">
                        <select name="game_type" id="srch_key">
                            <option value="all" <?php if($p_data['game_type']=='all') { echo "selected"; }?>>전체보기</option>
                            <?php if('ON' == IS_EOS_POWERBALL){ ?>
                                <option value="eospb5" <?php if($p_data['game_type']=='eospb5') { echo "selected"; }?>>EOS 파워볼</option>
                            <?php } ?>
                            <?php if('ON' == IS_POWERBALL){ ?>
                                <option value="powerball" <?php if($p_data['game_type']=='powerball') { echo "selected"; }?>>파워볼</option>
                            <?php } ?>
                            <option value="pladder" <?php if($p_data['game_type']=='pladder') { echo "selected"; }?>>파워사다리</option>
                            <option value="kladder" <?php if($p_data['game_type']=='kladder') { echo "selected"; }?>>키노사다리</option>
                        </select>
                    </div>
                    <div class="" style="padding-right: 10px;">
...
if($total_cnt > 0) {
    ...
    echo "<tr><td colspan='15'>데이터가 없습니다.</tr>";
}
?>
