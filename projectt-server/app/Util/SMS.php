...
*/
class SMS {
  //var $icode_key = "601b44adefefd10d4f08e40640fc23a8"; // 사무실
  ...
  function Add($strTelList, $strCallBack, $strData, $strSubject='', $strDate='') {
    ...
    */
    ...
    $strSubject = EscapeJsonString($strSubject);
    $strData = EscapeJsonString($strData);
    
    foreach ($strTelList as $tel) {
      if(empty($tel)) continue;
      $packet = sprintf('{"key":"%s","tel":"%s","cb":"%s","msg":"%s","title":"%s","date":"%s","charset":"utf-8"}', 
        $this->icode_key, $tel, $strCallBack, $strData, ($strSubject?$strSubject:''), ($strDate?$strDate:''));
