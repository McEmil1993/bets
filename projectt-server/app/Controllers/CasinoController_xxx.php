...
class CasinoController extends BaseController {
    ...
    public function index() {
        ...
        $query = "SELECT set_type, set_type_val FROM t_game_config WHERE set_type = 'service_casino' ";
        $result_game_config = $memberModel->db->query($query)->getResult();
    ...
    public function esports() {
        ...
        if (false == session()->has('member_idx') || !isset($member_idx) || false == session()->has('level')) {
            $url = base_url("/$viewRoot/index");
            echo "<script>
        	alert('로그인 후 이용해주세요.');
        	window.location.href='$url';
    ...
    public function slot() {
        ...
        $query = "SELECT set_type, set_type_val FROM t_game_config WHERE set_type = 'service_slot' ";
        $result_game_config = $memberModel->db->query($query)->getResult();
        if ('Y' == $result_game_config[0]->set_type_val && 9 != $member->getLevel()) {
