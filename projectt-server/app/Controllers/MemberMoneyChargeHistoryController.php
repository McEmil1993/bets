...
class MemberMoneyChargeHistoryController extends BaseController {
    ...
    public function chargeRequest() {
        ...
        try {
            ...
            //$this->logger->error('MemberMoneyChargeHistoryController chargeRequest array : ' . json_encode([$deposit_name,$chargeMoney,$str_account_number]));
            ...
            $memberMCHModel->chargeRequest(session()->get('account_name'), $chargeMoney, $findMember->getMoney(), $uKey, $result_bank_ids[0]['bank_id'], $str_account_number, $referenceId, $bonus_point, '', $display_account_bank, $account_name, $charge_point_yn, $bonus_idx, $level);
        ...
        try {
            ...
            foreach ($result_deposit_get as $result_get) {
                ...
                if ($bank_name == '수협') {
                    ...
                    $sql .= " where a.idx=b.member_idx and b.deposit_name = ? and b.money = ? AND b.status = 1 ORDER BY idx DESC limit 1 for update";
                    ...
                    if (4 != $chargeTypeData['charge_type']) {
                        $sql = "select a.idx, b.idx as charge_idx, a.level, a.money, b.money as set_money,a.point, a.is_exchange, a.reg_first_charge,a.charge_first_per,b.create_dt,b.u_key, b.charge_point_yn"
                                . ",b.bonus_level,b.bonus_option_idx"
                                . " from member a, member_money_charge_history b ";
                        $sql .= " where a.idx=b.member_idx and b.deposit_name = ? and b.money = ? and b.bank_id = ? AND b.status = 1 ORDER BY idx DESC limit 1 for update";
    ...
    private function checkSMS($memberMCHModel) {
        $sql = "select idx, deposit_name, bank_id, bank_name, money, create_dt from member_money_charge_sms ";
        $sql .= " where status = 1";
        $result_deposit_get = $memberMCHModel->db->query($sql)->getResultArray();
        if (0 >= count($result_deposit_get)) {
            $this->logger->error('do not auto_charge data : ' . $sql);
            return array(false, null);
        }
        return array(true, $result_deposit_get);
    }

    private function getEventData($memberMCHModel) {
        $sql = "select bonus, max_bonus from charge_event order by level asc;";
        $chargeEventData = $memberMCHModel->db->query($sql)->getResultArray();
        if (!isset($chargeEventData) || 0 >= count($chargeEventData)) {
            $this->logger->critical('fail get event data : ' . $sql);
            return [false, null];
        }

        return [true, $chargeEventData];
    }

    private function getChargeConfigData($memberMCHModel) {
        $db_config = array();
        $db_config_set_type = array();
        $get_config_str = "'charge_first_per', 'charge_per', 'charge_max_money', 'charge_money','reg_first_charge','event_charge_status','event_charge_start','event_charge_end' ";
        $sql = "select u_level, set_type, set_type_val from t_game_config ";
        $sql .= " where set_type in ($get_config_str) ";
        $retData = $memberMCHModel->db->query($sql)->getResultArray();
        foreach ($retData as $row) {
            ...
            switch ($row['set_type']) {
                ...
                case 'charge_money':
                    ...
                    break;
                case 'reg_first_charge':
                    $db_config_set_type['reg_first_charge'] = $row['set_type_val'];
                    break;
                case 'event_charge_status':
                    $db_config_set_type['event_charge_status'] = $row['set_type_val'];
                    break;
                case 'event_charge_start':
                    $db_config_set_type['event_charge_start'] = $row['set_type_val'];
                    break;
                case 'event_charge_end':
                    $db_config_set_type['event_charge_end'] = $row['set_type_val'];
                    break;
            }
        }

        return [$db_config, $db_config_set_type];
    }

    private function checkChargeTime($deposit_get_idx, $currentDate, $result_get, $memberMCHModel) {
        $endTime = date("Y-m-d H:i:s", strtotime($result_get['create_dt'] . "+" . 15 . " minutes"));
        if ($endTime <= $currentDate) {
            $sql = "update member_money_charge_sms set status = 2 where idx = ?";
            $memberMCHModel->db->query($sql, [$deposit_get_idx]);
            $this->logger->critical('::::::::::::::: auto_money_charge time out fail ' . $endTime . ' : ' . $result_get[0]['create_dt']);
            return false;
        }
        return true;
    }

    private function checkPersonalAccount($deposit_name, $money, $bank_id, $memberMCHModel) {
        if ($bank_name == '수협') {
            $sql = "select a.idx, b.idx as charge_idx, a.level, a.money, b.money as set_money,a.point, a.is_exchange, a.reg_first_charge,a.charge_first_per,b.create_dt,b.u_key, b.charge_point_yn"
                    . ",b.bonus_level,b.bonus_option_idx"
                    . " from member a, member_money_charge_history b ";
            $sql .= " where a.idx=b.member_idx and b.deposit_name = ? and b.money = ? AND b.status = 1 ORDER BY idx DESC limit 1 for update";
            $retUsersData = $memberMCHModel->db->query($sql, [$deposit_name, $money])->getResultArray();
            if (!isset($retUsersData) || 0 == count($retUsersData)) {
                //$this->logger->debug(':::::::: do not charge_history data');
                return [false, null];
            }

            $sql = "select charge_type from charge_type where level = ?";
            $chargeTypeData = $memberMCHModel->db->query($sql, [$retUsersData[0]['level']])->getResultArray()[0];
            if (4 != $chargeTypeData['charge_type']) {
                $sql = "select a.idx, b.idx as charge_idx, a.level, a.money, b.money as set_money,a.point, a.is_exchange, a.reg_first_charge,a.charge_first_per,b.create_dt,b.u_key, b.charge_point_yn"
                        . ",b.bonus_level,b.bonus_option_idx"
                        . " from member a, member_money_charge_history b ";
                $sql .= " where a.idx=b.member_idx and b.deposit_name = ? and b.money = ? and b.bank_id = ? AND b.status = 1 ORDER BY idx DESC limit 1 for update";
                $retUsersData = $memberMCHModel->db->query($sql, [$deposit_name, $money, $bank_id])->getResultArray();
            }
        } else {
            $sql = "select a.idx, b.idx as charge_idx, a.level, a.money, b.money as set_money,a.point, a.is_exchange, a.reg_first_charge,a.charge_first_per,b.create_dt,b.u_key, b.charge_point_yn"
                    . ",b.bonus_level,b.bonus_option_idx"
        ...
        return [true, $retUsersData];
    }

    private function updateMaxCharge($m_idx, $set_money, $memberMCHModel) {
        $sql_tot = "SELECT max_charge FROM total_member_cash WHERE member_idx = ?";
        $max_charge_arr = $memberMCHModel->db->query($sql_tot, [$m_idx])->getResultArray();
        if (true === isset($max_charge_arr) && 0 < count($max_charge_arr)) {
            $max_charge = $max_charge_arr[0]['max_charge'];
    ...
    private function updateCharge($set_money, $ch_point, $now_point, $now_cash, $str_set_type, $af_cash, $deposit_get_idx, $const_value, $ratio_value, $charge_idx, $memberMCHModel) {
        $sql = "update member_money_charge_history set bonus_point = ?,set_type = ?, status = 3, update_dt=now(), result_money = ?, referenceId= ? 
				,const_value = ? ,ratio_value = ? 
				where idx = ?";
        // 로그에는 두개합산값으로 저장한다.
        ...
        return [$set_money, $af_point, $af_cash];
    }

    private function doCharge($result_get, $db_config, $chargeEventData, $db_config_set_type, $memberMCHModel) {
        $deposit_get_idx = $result_get['idx'];
        $deposit_name = $result_get['deposit_name'];
        $money = $result_get['money'];
        $bank_id = $result_get['bank_id'];
        $bank_name = $result_get['bank_name'];
        // 입금 시간 체크 
        if (false == $this->checkChargeTime($deposit_get_idx, $currentDate, $result_get, $memberMCHModel)) {
            return;
        }

        // 개인계좌 체크
        list($retval, $retUsersData) = $this->checkPersonalAccount($deposit_name, $money, $bank_id, $memberMCHModel);
        if (false == $retval) {
            return;
        }
        $ratio_value = 0;
        $const_value = 0;
        // 이벤트 충전->가입 첫충->첫충-매충 순서로 보상지급
        // 가입첫충,이벤트 첫충중 비율이 높은게 먼저 적용된다.
        // 첫충,매충일때만 보너스 선택창이 뜬다.
        if (0 == $retUsersData[0]['reg_first_charge']) { // 가입 첫충
            list($p_a_comment, $str_set_type, $ch_point) = $this->regFirstCharge($db_config, $retUsersData, $chargeEventData, $db_config_set_type, $memberMCHModel);
        } else if ($db_config_set_type['event_charge_status'] == 'ON' && date("Y-m-d " . $db_config_set_type['event_charge_start']) <= $currentDate && date("Y-m-d " . $db_config_set_type['event_charge_end']) >= $currentDate &&
                $chargeEventData[$u_level - 1]['bonus'] > 0) { // 이벤트 충전기간이다
            list($p_a_comment, $str_set_type, $ch_point) = $this->eventCharge($retUsersData, $chargeEventData, $memberMCHModel);
        } else if (0 == $retUsersData[0]['charge_first_per']) { //첫충
            list($p_a_comment, $str_set_type, $ch_point, $ratio_value, $const_value) = $this->dayFirstCharge($retUsersData, $chargeEventData, $memberMCHModel);
        } else { // 매충 
            list($p_a_comment, $str_set_type, $ch_point, $ratio_value, $const_value) = $this->everyCharge($retUsersData, $chargeEventData, $memberMCHModel);
        }

        UserPayBack::AddCharge($retUsersData[0]['idx'], $retUsersData[0]['set_money'], $memberMCHModel);
        // 입금성공한 금액이 최대 입금금액 보다 많으면 데이터를 갱신해준다.
        $this->updateMaxCharge($retUsersData[0]['idx'], $retUsersData[0]['set_money'], $memberMCHModel);

        list($result_set_money, $af_point, $af_cash) = $this->updateCharge($retUsersData[0]['set_money'], $ch_point, $retUsersData[0]['point'], $retUsersData[0]['money'], $str_set_type, $af_cash, $deposit_get_idx, $const_value, $ratio_value, $retUsersData[0]['charge_idx'], $memberMCHModel);

        $this->gmPt->giveGMoneyCharge($memberMCHModel, $retUsersData[0]['u_key'], $retUsersData[0]['idx'], $result_set_money, $retUsersData[0]['money'], $af_cash
                , $ch_point, $retUsersData[0]['point'], $af_point, $p_a_comment, $retUsersData[0]['charge_idx'], $this->logger);
    }

    public function auto_money_charge_renew() { // 10초에 한번씩 호출된다.
        try {
            //$this->logger->debug('::::::::::::::: auto_money_charge start');
            $memberMCHModel = new MemberMoneyChargeHistoryModel();
            $memberMCHModel->db->transStart();
            // 입금내역 문자가 있는지 확인한다.
            list($smsResult, $result_deposit_get) = $this->checkSMS($memberMCHModel);
            if (false == $smsResult) {
            ...
            // 돌발충전 보너스 데이터 얻어오기 
            list($retval, $chargeEventData) = $this->getEventData($memberMCHModel);
            if (false == $retval) {
                $memberMCHModel->db->transRollback();
                return;
            }

            list($db_config, $db_config_set_type) = $this->getChargeConfigData($memberMCHModel);

            //$this->logger->debug('::::::::::::::: auto_money_charge result_deposit_get ' . json_encode($result_deposit_get));
            $currentDate = date("Y-m-d H:i:s");

            foreach ($result_deposit_get as $result_get) {
                $this->doCharge($result_get, $db_config, $chargeEventData, $db_config_set_type, $memberMCHModel);
            }
    ...
    private function regFirstCharge($db_config, $retUsersData, $chargeEventData, $db_config_set_type, $memberMCHModel) {

        $u_level = $retUsersData[0]['level'];
        $n_reg_first_charge = $db_config_set_type['reg_first_charge'];
        if (1 == $retUsersData[0]['charge_point_yn']) {
            throw new Exception('가입첫충시 보너스 선택할 수 없습니다.');
        }

        if ($db_config_set_type['event_charge_status'] == 'ON' && date("Y-m-d " . $db_config_set_type['event_charge_start']) <= $currentDate && date("Y-m-d " . $db_config_set_type['event_charge_end']) >= $currentDate &&
                $chargeEventData[$u_level - 1]['bonus'] > 0) {
            if ($chargeEventData[$u_level - 1]['bonus'] > $n_reg_first_charge) {
                $n_reg_first_charge = $chargeEventData[$u_level - 1]['bonus'];
            }
        }

        $ch_point = ($n_reg_first_charge * $retUsersData[0]['set_money']) / 100;
        if ($db_config[$u_level]['charge_max_money'] < $ch_point) {
            $ch_point = $db_config[$u_level]['charge_max_money'];
        }
        $p_a_comment = '포인트 충전 : ' . 'reg_first_charge';
        ...
        $memberMCHModel->db->query($sql, [$retUsersData[0]['set_money'], $ch_point, $retUsersData[0]['idx']]);
        return [$p_a_comment, $str_set_type, $ch_point];
    }

    private function eventCharge($retUsersData, $chargeEventData, $memberMCHModel) {

        if (1 == $retUsersData[0]['charge_point_yn']) {
            throw new Exception('이벤트 충전시 보너스 선택할 수 없습니다.');
        }
        $u_level = $retUsersData[0]['level'];
        $ch_point = ($chargeEventData[$u_level - 1]['bonus'] * $retUsersData[0]['set_money']) / 100;
        if ($chargeEventData[$u_level - 1]['max_bonus'] < $ch_point) {
            $ch_point = $chargeEventData[$u_level - 1]['max_bonus'];
        }
        $p_a_comment = '포인트 돌발충전 : ' . 'charge_event_per';
    ...
    private function dayFirstCharge($retUsersData, $chargeEventData, $memberMCHModel) {
        $bonus_level = $retUsersData[0]['bonus_level'];
        $bonus_option_idx = $retUsersData[0]['bonus_option_idx'];

        list($ch_point, $ratio_value, $const_value) = $this->do_select_charge_first($bonus_level, $bonus_option_idx, $retUsersData[0]['set_money'], $memberMCHModel);
        ...
        $sql = "update member set charge_first_per = 1,money = money + ?, point = point + ? where idx = ? ";
        $memberMCHModel->db->query($sql, [$retUsersData[0]['set_money'], $ch_point, $retUsersData[0]['idx']]);
        return [$p_a_comment, $str_set_type, $ch_point, $ratio_value, $const_value];
    }

    private function everyCharge($retUsersData, $chargeEventData, $memberMCHModel) {
        $bonus_level = $retUsersData[0]['bonus_level'];
        $bonus_option_idx = $retUsersData[0]['bonus_option_idx'];

        list($ch_point, $ratio_value, $const_value) = $this->do_select_charge($bonus_level, $bonus_option_idx, $retUsersData[0]['set_money'], $memberMCHModel);
